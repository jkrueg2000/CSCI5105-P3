...
def _row_to_item(item_id, row) -> kv_pb2.MarketplaceItem:
    item = kv_pb2.MarketplaceItem()
...
class StorageService(kv_pb2_grpc.StorageServiceServicer):
    def CreateItem(self, request, context):
        item = request.item
        with DATA_LOCK:
            if item.item_id in DATA_DF.index:
                existing = DATA_DF.loc[item.item_id]
                return kv_pb2.ActionResponse(success=False, message="item exists", new_version=int(existing.get("version", 0) or 0))
        ...
        return kv_pb2.ActionResponse(success=True, message="created", new_version=version)
    ...
    def SearchItems(self, request, context):
        ...
        return kv_pb2.SearchResponse(items=results)

    def UpdateItem(self, request, context):
        item_id = request.item_id
        with DATA_LOCK:
            if item_id not in DATA_DF.index:
                return kv_pb2.ActionResponse(success=False, message="not found", new_version=0)
            ...
            if request.expected_version and request.expected_version != current_version:
                return kv_pb2.ActionResponse(success=False, message="version mismatch", new_version=current_version)
            if has_desc:
        ...
        return kv_pb2.ActionResponse(success=True, message="updated", new_version=new_version)
    ...
    def PlaceBid(self, request, context):
        ...
        with DATA_LOCK:
            ...
            if status != "AUCTION_ACTIVE":
                return kv_pb2.ActionResponse(success=False, message="auction not active", new_version=version)
            min_price = current_price or starting_price
            if request.bid_amount <= min_price:
                return kv_pb2.ActionResponse(success=False, message="bid too low", new_version=version)
        ...
        return kv_pb2.ActionResponse(success=True, message="bid accepted", new_version=new_version)

    def AuctionPoll(self, request, context):
        item_id = request.item_id
        with DATA_LOCK:
            if item_id not in DATA_DF.index:
                return kv_pb2.AuctionEvent(type=kv_pb2.AuctionEvent.STATUS_CHANGE, item_snapshot=kv_pb2.MarketplaceItem(), event_description="not found")
            row = DATA_DF.loc[item_id]
            item = _row_to_item(item_id, row)
            return kv_pb2.AuctionEvent(type=kv_pb2.AuctionEvent.STATUS_CHANGE, item_snapshot=item, event_description="polled")


def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    kv_pb2_grpc.add_StorageServiceServicer_to_server(StorageService(), server)
